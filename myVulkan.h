...
const std::string TEXTURE_PATH = "source/fundo.jpg";
...
struct MyTextureData
{
    ...
    void carregar(bool carregar) { noLoad = carregar; }

    ImVec2 tamanho(int altera_altura = (0), int altera_largura = (0)) { return ImVec2(this->Width-altera_altura, this->Height-altera_largura);}
};
    ...
    void createTextureImage(const char* textura, MyTextureData& tex_data) {
        ...
        tex_data.Height = texHeight;
        tex_data.Width = texWidth;
        tex_data.Channels = texChannels;
        ...
        createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
            ...
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, tex_data.Image, tex_data.ImageMemory);


        transitionImageLayout(tex_data.Image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        copyBufferToImage(temp, tex_data.Image, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
        transitionImageLayout(tex_data.Image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        ...
        createTextureImageView(tex_data.Image, &tex_data.ImageView);
        createTextureSampler(&tex_data.Sampler);
    ...
    void destroir_image(MyTextureData *tex_data) {
        vkFreeMemory(vkDevice, tex_data->UploadBufferMemory, nullptr);
        vkDestroyBuffer(vkDevice, tex_data->UploadBuffer, nullptr);
        vkDestroySampler(vkDevice, tex_data->Sampler, nullptr);
        vkDestroyImageView(vkDevice, tex_data->ImageView, nullptr);
        vkDestroyImage(vkDevice,  tex_data->Image, nullptr);
        vkFreeMemory(vkDevice, tex_data->ImageMemory, nullptr);
...
public:
 ...
 1.0f - attrib.texcoords[2 * index.texcoord_index + 1]
    ...
    void mainLoop(VkCommandBuffer func(vulkan &app, int imageindex)) {
        ...
        using framerate = std::chrono::duration<int, std::ratio<1, 16>>;
